...
open class CoordinatorMock: Coordinator, Mock {
    ...
    open func goToCarMap() {
        addInvocation(.m_goToCarMap)
		let perform = methodPerformValue(.m_goToCarMap) as? () -> Void
		perform?()
    }
    ...
    fileprivate enum MethodType {
        ...
        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Bool {
            ...
            case (.m_goToCarList, .m_goToCarList):
                return true 
            case (.m_goToCarMap, .m_goToCarMap):
                return true 
    ...
    public struct Verify {
        ...
        public static func goToCarMap() -> Verify { return Verify(method: .m_goToCarMap)}
    }
    ...
    public struct Perform {
        ...
        public static func goToCarMap(perform: @escaping () -> Void) -> Perform {
            return Perform(method: .m_goToCarMap, performs: perform)
        }
    }
...
// MARK: - GetPlaceMarks
open class GetPlaceMarksMock: GetPlaceMarks, Mock {
    init(sequencing sequencingPolicy: SequencingPolicy = .lastWrittenResolvedFirst, stubbing stubbingPolicy: StubbingPolicy = .wrap, file: StaticString = #file, line: UInt = #line) {
        SwiftyMockyTestObserver.setup()
        self.sequencingPolicy = sequencingPolicy
        self.stubbingPolicy = stubbingPolicy
    ...
    open func execute() throws -> PlaceMarks {
        addInvocation(.m_execute)
		let perform = methodPerformValue(.m_execute) as? () -> Void
		perform?()
		var __value: PlaceMarks
		do {
		    __value = try methodReturnValue(.m_execute).casted()
    ...
    open class Given: StubbedMethod {
        ...
        public static func execute(willReturn: PlaceMarks...) -> MethodStub {
            return Given(method: .m_execute, products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        ...
        public static func execute(willProduce: (StubberThrows<PlaceMarks>) -> Void) -> MethodStub {
            let willThrow: [Error] = []
			let given: Given = { return Given(method: .m_execute, products: willThrow.map({ StubProduct.throw($0) })) }()
			let stubber = given.stubThrows(for: (PlaceMarks).self)
			willProduce(stubber)
			return given
        }
    }
