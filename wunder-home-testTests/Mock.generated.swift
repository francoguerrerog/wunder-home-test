...
open class CoordinatorMock: Coordinator, Mock {
    ...
    open func goToCarList() {
        addInvocation(.m_goToCarList)
		let perform = methodPerformValue(.m_goToCarList) as? () -> Void
		perform?()
    }

    open func goToCarMap(placeMark: PlaceMark) {
        addInvocation(.m_goToCarMap__placeMark_placeMark(Parameter<PlaceMark>.value(`placeMark`)))
		let perform = methodPerformValue(.m_goToCarMap__placeMark_placeMark(Parameter<PlaceMark>.value(`placeMark`))) as? (PlaceMark) -> Void
		perform?(`placeMark`)
    }
    ...
    fileprivate enum MethodType {
        case m_goToCarList
        case m_goToCarMap__placeMark_placeMark(Parameter<PlaceMark>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Bool {
            switch (lhs, rhs) {
            case (.m_goToCarList, .m_goToCarList):
                return true 
            case (.m_goToCarMap__placeMark_placeMark(let lhsPlacemark), .m_goToCarMap__placeMark_placeMark(let rhsPlacemark)):
                guard Parameter.compare(lhs: lhsPlacemark, rhs: rhsPlacemark, with: matcher) else { return false } 
                return true 
        ...
        func intValue() -> Int {
            switch self {
            case .m_goToCarList: return 0
            case let .m_goToCarMap__placeMark_placeMark(p0): return p0.intValue
            }
        }
    }
    ...
    public struct Verify {
        ...
        public static func goToCarList() -> Verify { return Verify(method: .m_goToCarList)}
        public static func goToCarMap(placeMark: Parameter<PlaceMark>) -> Verify { return Verify(method: .m_goToCarMap__placeMark_placeMark(`placeMark`))}
    }
    ...
    public struct Perform {
        ...
        public static func goToCarList(perform: @escaping () -> Void) -> Perform {
            return Perform(method: .m_goToCarList, performs: perform)
        }
        public static func goToCarMap(placeMark: Parameter<PlaceMark>, perform: @escaping (PlaceMark) -> Void) -> Perform {
            return Perform(method: .m_goToCarMap__placeMark_placeMark(`placeMark`), performs: perform)
        }
    }
